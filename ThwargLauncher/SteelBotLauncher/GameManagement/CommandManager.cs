...
namespace SteelBotLauncher
{
    ...
    class CommandManager
    {
        ...
        private void HandleAddLoginCmdCommand(GameSession session, string commandString)
        {
            ...
            var cmds = SteelFilter.LoginCommandsStorage.GetLoginCommands(session.AccountName, session.ServerName, session.CharacterName);
            ...
            SteelFilter.LoginCommandsStorage.SetLoginCommands(session.AccountName, session.ServerName, session.CharacterName, newCmdListString, waitTimeMs);
        }
        ...
        {
            var filterCmd = new SteelFilter.Channels.Command(DateTime.UtcNow, cmdtext);
