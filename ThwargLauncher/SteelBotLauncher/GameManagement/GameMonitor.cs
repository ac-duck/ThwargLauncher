...
namespace SteelBotLauncher
{
    ...
    class GameMonitor
    {
        ...
        {
            foreach (string filepath in SteelFilter.FileLocations.GetAllCharacterFilePaths())
            {
        ...
        string filepath, SteelFilter.LaunchControl.HeartbeatResponse response)
        {
        ...
        {
            DirectoryInfo dir = new DirectoryInfo(SteelFilter.FileLocations.GetRunningFolder());
            ...
            foreach (var fileInfo in dir.EnumerateFiles("game_*.txt"))
            {
                ...
                {
                    processId = SteelFilter.FileLocations.GetProcessIdFromGameHeartbeatFilepath(fileInfo.Name);
